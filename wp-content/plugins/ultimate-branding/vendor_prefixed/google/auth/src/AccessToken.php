 ...
 */
namespace Branda_Vendor\Google\Auth;

use DateTime;
use Exception;
use Branda_Vendor\Firebase\JWT\ExpiredException;
use Branda_Vendor\Firebase\JWT\JWT;
use Branda_Vendor\Firebase\JWT\SignatureInvalidException;
use Branda_Vendor\Google\Auth\Cache\MemoryCacheItemPool;
use Branda_Vendor\Google\Auth\HttpHandler\HttpClientCache;
use Branda_Vendor\Google\Auth\HttpHandler\HttpHandlerFactory;
use Branda_Vendor\GuzzleHttp\Psr7\Request;
use Branda_Vendor\GuzzleHttp\Psr7\Utils;
use InvalidArgumentException;
use Branda_Vendor\phpseclib\Crypt\RSA;
use Branda_Vendor\phpseclib\Math\BigInteger;
use Branda_Vendor\Psr\Cache\CacheItemPoolInterface;
use RuntimeException;
use Branda_Vendor\SimpleJWT\InvalidTokenException;
use Branda_Vendor\SimpleJWT\JWT as SimpleJWT;
use Branda_Vendor\SimpleJWT\Keys\KeyFactory;
use Branda_Vendor\SimpleJWT\Keys\KeySet;
...
class AccessToken
{
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\Branda_Vendor\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\Branda_Vendor\SignatureInvalidException $e) {
            // firebase/php-jwt 2
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('Branda_Vendor\\SimpleJWT\\JWT')) {
    ...
    {
        $class = 'Branda_Vendor\\Firebase\\JWT\\JWT';
