 ...
 */
namespace Branda_Vendor\Google\AccessToken;

use Branda_Vendor\Firebase\JWT\ExpiredException as ExpiredExceptionV3;
use Branda_Vendor\Firebase\JWT\SignatureInvalidException;
use Branda_Vendor\GuzzleHttp\Client;
use Branda_Vendor\GuzzleHttp\ClientInterface;
use Branda_Vendor\phpseclib3\Crypt\PublicKeyLoader;
use Branda_Vendor\phpseclib3\Crypt\RSA\PublicKey;
use Branda_Vendor\Psr\Cache\CacheItemPoolInterface;
use Branda_Vendor\Google\Auth\Cache\MemoryCacheItemPool;
use Branda_Vendor\Google\Exception as GoogleException;
use Branda_Vendor\Stash\Driver\FileSystem;
use Branda_Vendor\Stash\Pool;
use DateTime;
use DomainException;
use Exception;
use Branda_Vendor\ExpiredException;
// Firebase v2
use LogicException;
/**
...
class Verify
{
    ...
    {
        $jwtClass = 'JWT';
        if (\class_exists('Branda_Vendor\\Firebase\\JWT\\JWT')) {
            $jwtClass = 'Branda_Vendor\\Firebase\\JWT\\JWT';
        }
    ...
    private function getPublicKey($cert)
    {
        ...
        if (\class_exists('Branda_Vendor\\phpseclib3\\Crypt\\RSA\\PublicKey')) {
    ...
    {
        if (\class_exists('Branda_Vendor\\phpseclib3\\Crypt\\RSA')) {
            return 'Branda_Vendor\\phpseclib3\\Crypt\\RSA';
        }
        if (\class_exists('Branda_Vendor\\phpseclib\\Crypt\\RSA')) {
            return 'Branda_Vendor\\phpseclib\\Crypt\\RSA';
        }
        return 'Crypt_RSA';
    }
    private function getBigIntClass()
    {
        if (\class_exists('Branda_Vendor\\phpseclib3\\Math\\BigInteger')) {
            return 'Branda_Vendor\\phpseclib3\\Math\\BigInteger';
        }
        if (\class_exists('Branda_Vendor\\phpseclib\\Math\\BigInteger')) {
            return 'Branda_Vendor\\phpseclib\\Math\\BigInteger';
        }
        return 'Math_BigInteger';
    }
    private function getOpenSslConstant()
    {
        if (\class_exists('Branda_Vendor\\phpseclib3\\Crypt\\AES')) {
            return 'phpseclib3\\Crypt\\AES::ENGINE_OPENSSL';
        }
        if (\class_exists('Branda_Vendor\\phpseclib\\Crypt\\RSA')) {
            return 'phpseclib\\Crypt\\RSA::MODE_OPENSSL';
        }
        if (\class_exists('Branda_Vendor\\Crypt_RSA')) {
