...
namespace Branda_Vendor\GuzzleHttp\Handler;

use Branda_Vendor\GuzzleHttp\Exception\ConnectException;
use Branda_Vendor\GuzzleHttp\Exception\RequestException;
use Branda_Vendor\GuzzleHttp\Promise\FulfilledPromise;
use Branda_Vendor\GuzzleHttp\Promise\PromiseInterface;
use Branda_Vendor\GuzzleHttp\Psr7;
use Branda_Vendor\GuzzleHttp\TransferStats;
use Branda_Vendor\GuzzleHttp\Utils;
use Branda_Vendor\Psr\Http\Message\RequestInterface;
use Branda_Vendor\Psr\Http\Message\ResponseInterface;
use Branda_Vendor\Psr\Http\Message\StreamInterface;
/**
...
class StreamHandler
{
    ...
    public function __invoke(RequestInterface $request, array $options)
    {
        ...
        try {
            ...
            return \Branda_Vendor\GuzzleHttp\Promise\rejection_for($e);
        }
    }
    ...
    private function createResponse(RequestInterface $request, array $options, $stream, $startTime)
    {
        ...
        $headers = \Branda_Vendor\GuzzleHttp\headers_from_lines($hdrs);
        ...
        if (isset($options['on_headers'])) {
            try {
                ...
                return \Branda_Vendor\GuzzleHttp\Promise\rejection_for($ex);
            }
        }
    ...
    private function checkDecode(array $options, array $headers, $stream)
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \Branda_Vendor\GuzzleHttp\normalize_header_keys($headers);
    ...
    private function add_proxy(RequestInterface $request, &$options, $value, &$params)
    {
        if (!\is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no']) || !\Branda_Vendor\GuzzleHttp\is_host_in_noproxy($request->getUri()->getHost(), $value['no'])) {
    ...
    private function add_verify(RequestInterface $request, &$options, $value, &$params)
    {
        if ($value === \true) {
            ...
            if (\PHP_VERSION_ID < 50600) {
                $options['ssl']['cafile'] = \Branda_Vendor\GuzzleHttp\default_ca_bundle();
            }
    ...
    private function add_debug(RequestInterface $request, &$options, $value, &$params)
    {
        ...
        $value = \Branda_Vendor\GuzzleHttp\debug_resource($value);
